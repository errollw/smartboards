var settings_title_fmt = '<i class="icon-gears"></i>&nbsp; Room Settings',
    settings_title_fmt_mobile = '{0} Settings',
    settings_subtitle_fmt = 'Room ID : &nbsp; <span class:"mono">{0}</span> &ensp; <i class="icon-edit"></i>&nbsp; {1}',
...
function save_settings() {
    ...
    $.post(SCRIPTS_FMT.f('make_room'), {
        'order-of-users': JSON.stringify(order_of_users),
    ...
    // location.reload();
    // TODO: keep track of what's been uploaded, and refresh
}
...
function init_settings(){
    ...
    // On clicking a user setting 'trash' button, remove its UI elements
    $settings_container.on('click', 'button.trash', function(){
        $(this).parent().remove();                                      // FIXME: awkward DOM traversal
    });

    // Can reset each user's SVG
    $settings_container.on('click', 'button.reset-svg', function(){
        var u_id = $(this).parent().find('input[name=u_id]').val();     // FIXME: awkward DOM traversal
        reset_svg(u_id);
    });
