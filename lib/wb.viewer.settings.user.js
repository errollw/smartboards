function make_user_settings_div(){	
    ...
    var $prof_pic_btn =  $('<button/>', { class: 'profile-pic' }).html('<i class="icon-picture"></i>'),
        $trash_btn =     $('<button/>', { class: 'red trash' }).html('<i class="icon-trash"></i> &ensp; Delete user')
        $reset_btn =     $('<button/>', { class: 'red reset-svg' }).html('<i class="icon-reply-all"></i> &ensp; Reset SVG'),
        $hidden_upload = $('<form/>', {
    ...
    $u_id.append('<p>User ID</p><input type="text" name="u_id" class="mono" placeholder="unique identifier">').append($prof_pic_btn);
    ...
    $u_settings.append($hidden_upload).append($reset_btn).append($trash_btn);;
