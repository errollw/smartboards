...
var anchor_pt = {x:0, y:0},
    ...
    resize_diag = false;                // TRUE if is resizing via a diagonal
...
function make_movable_drawing(image_src, image_box, $wb_pane){
    ...
    var img_aspect_ratio = image_box.width/image_box.height;
    ...
    // Set an initial autoplace timeout for if user forgets to move or resize image
    set_autoplace_timeout($m_d, $wb_pane);
    ...
    $.extend($m_d, {
        ...
        stop: function(){
            if (is_dragging || is_resizing) {
                ...
                set_autoplace_timeout($m_d, $wb_pane);         // Set m_d to place itself after a timeout
        ...
        resize_to: function(mouseX, mouseY){
            ...
            $m_d.height( Math.round($m_d.width() / img_aspect_ratio));
        ...
        place: function() {
            window.clearTimeout(autoplace_timeout); // Cancel any on-going auto-place timeouts!
...
function set_autoplace_timeout($m_d, $wb_pane){
    window.clearTimeout(autoplace_timeout);
    autoplace_timeout = window.setTimeout(
        function(){
...
function add_control_points($m_d){
    ...
    // Link control points to their opposites
    for(var i=0; i<3; i++){
        for(var j=0; j<3; j++){
...
//Simplified handlers for kiosk (mouse only)
