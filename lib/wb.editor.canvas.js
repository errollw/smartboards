...
function open_canvas($wb_content_div) {
    ...
    function resize(){
        ...
        $container.css({
            width: $wb_content_div.width() - parseInt($container.css('margin'))*2,
            height: $wb_content_div.height() - parseInt($container.css('margin'))*2,
...
function canvas_init($canvas) {
    ...
    $.extend($canvas, {
        ...
        to_movable_drawing : function($wb_pane, target_svg_wrapper){
            ...
            var $movable_drawing =
                make_movable_drawing(data_URI, image_box, parent_rect, $wb_pane);
