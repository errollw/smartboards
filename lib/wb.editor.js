...
function wb_pane($container, user) {
    ...
    $header.on('mouseup touchend', 'button.upload', function(e) {       // Must be done on mouseup before it reaches the movable handler
        e.stopPropagation();                // Prevent placing when clicking $wb-pane
        $movable_drawing.place();
        $new_wb_pane.set_edit_mode('idle');
    });

    $header.on('mouseup touchend', 'button.trash', function(e) {
        e.stopPropagation();
