...
function add_wb_pane(user) {
    ...
    var $header =       $('<div/>', {class: 'wb-header' }),
        ...
        $edit_btn =     $('<button/>').html('Editor'),
        $settings_btn = $('<button/>', {class: 'show-settings desktop-only'}).html('Settings');
    $description.append($title).append($subtitle);
    $header.append($profile_pic).append($description).append($edit_btn).append($settings_btn);
