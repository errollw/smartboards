function place_img_in_svg($img, svg_wrapper){

    // slice URI to remove 'url(' and ')' parts of background-image (maybe a slow operation)
    img_data_uri = $img.css('background-image').slice(4,-1)
    ...
    var pos = {
        x: ($img.position().left + M_D_BORDER_WIDTH) * zoom,
        y: ($img.position().top + M_D_BORDER_WIDTH) * zoom
    };

    image = svg_wrapper.image(pos.x, pos.y, size.w, size.h, img_data_uri);

    // Ensure placed image can be clicked (parent SVG can be clicked through to underlying iframe)
